...
df[1653, 12] = "Planning"
which(df$construct2=="Other (Whoever vendors are…)")
...
d2 <-ggplot(df2, aes(x = year, y = proportion_change, group = condomain, color = condomain)) +
    geom_line() +
    labs(x = "Year", y = "Δ", title = "Change in proportion of condomains from previous year") +
  geom_point(size=3) +
  theme_bw()
...
d3 <- ggplot(df3, aes(x = year, y = proportion_change, group = condomain, color = condomain)) +
    geom_line() +
    geom_point(size=3) +
    labs(x = "Year", y = "Δ", title = "Change in proportion of condomain per year from baseline") +
    theme_bw()
...
v <- df %>%
    ...
    arrange(condomain, conconstruct) %>%
    print(n = 60)
write.csv(v, file = "consensusconconstructprop_WB071124.csv", row.names = TRUE)

## ordered table 
contable <- df %>%
    group_by(condomain, conconstruct) %>% 
    arrange(desc(condomain), desc(conconstruct))
write.csv(contable, file = "orderedtable_WB071124.csv", row.names = TRUE)
